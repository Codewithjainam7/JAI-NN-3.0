          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 mt-12 pt-12 border-t border-white/5">
            ...
            <div className="prose prose-invert animate-slide-up" style={{ animationDelay: '0.5s' }}>
              ...
              <div className="mt-6">
                ...
                {/* Contact block: show all relevant contact info (email, phone, website, socials) */}
                <div className="p-4 rounded-xl bg-black/20 border border-white/6">
                  {CREATOR_INFO.contact && (
                    <p className="text-sm text-white/70 mb-2">{CREATOR_INFO.contact}</p>
                  )}
                  ...
                  <div className="text-sm text-white/60 space-y-2">
                    {CREATOR_INFO.email && (
                      <div className="flex items-center gap-3">
                        ...
                        <a className="underline text-white/80" href={`mailto:${CREATOR_INFO.email}`}>{CREATOR_INFO.email}</a>
                      </div>
                    )}
                    ...
                    {CREATOR_INFO.phone && (
                      <div className="flex items-center gap-3">
                        ...
                        <a className="text-white/80" href={`tel:${CREATOR_INFO.phone}`}>{CREATOR_INFO.phone}</a>
                      </div>
                    )}
                    ...
                    {CREATOR_INFO.website && (
                      <div className="flex items-center gap-3">
                        ...
                        <a className="text-white/80 underline" href={CREATOR_INFO.website} target="_blank" rel="noreferrer">{CREATOR_INFO.website}</a>
                      </div>
                    )}

                    {/* Social icons if available */}
                    <div className="flex items-center gap-3 mt-3 flex-wrap">
                      {CREATOR_INFO.twitter && (
                        <a href={CREATOR_INFO.twitter} target="_blank" rel="noreferrer" className="px-2 py-1 rounded-md bg-white/5 hover:bg-white/10">
                          <Icon name="twitter" size={16} />
                        </a>
                      )}
                      {CREATOR_INFO.linkedin && (
                        <a href={CREATOR_INFO.linkedin} target="_blank" rel="noreferrer" className="px-2 py-1 rounded-md bg-white/5 hover:bg-white/10">
                          <Icon name="linkedin" size={16} />
                        </a>
                      )}
                      {CREATOR_INFO.github && (
                        <a href={CREATOR_INFO.github} target="_blank" rel="noreferrer" className="px-2 py-1 rounded-md bg-white/5 hover:bg-white/10">
