        ...
        <div className={`flex flex-col h-full p-4 w-72 ${!isOpen ? 'md:hidden' : ''}`}>
            ...
            <div className="flex items-center justify-between mb-8 px-2 pt-2">
                 <div className="flex items-center gap-3">
                    ...
                    <h1 className="font-semibold text-lg tracking-tight text-white font-sans">JAI-NN</h1>
                 </div>
            ...
            <button 
                ...
                className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white/5 hover:bg-white/10 transition-colors mb-6 text-sm font-medium text-white border border-white/5 group shadow-sm font-sans"
            >
            ...
            <div className="flex-1 overflow-y-auto ios-scrollbar -mx-2 px-2">
                <div className="text-xs font-medium text-white/40 px-4 mb-2 uppercase tracking-wider sticky top-0 bg-black/90 backdrop-blur-sm py-1 z-10 font-sans">History</div>
                <div className="space-y-0.5">
                    {sessions.length === 0 ? (
                      <div className="px-4 py-8 text-center">
                          ...
                          <p className="text-sm text-white/30 font-sans">No chat history</p>
                      </div>
                    ) : (
                        ...
                        <div key={session.id} className="group relative">
                            {editingId === session.id ? (
                                <div className="px-2 py-1">
                                    <input
                                        autoFocus
                                        className="w-full bg-white/10 text-white text-sm px-3 py-2 rounded-lg outline-none border border-blue-500 focus:ring-1 focus:ring-blue-500 font-sans"
                                        value={editTitle}
                                ...
                                <button
                                    ...
                                    className={`w-full text-left px-4 py-3 rounded-xl text-sm truncate transition-all duration-200 relative z-0 font-sans ${
            ...
            <div className="mt-auto pt-4 space-y-2 border-t border-white/10">
                 <button 
                   ...
                   className="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl hover:bg-white/5 text-white/60 hover:text-white transition-colors text-sm font-medium font-sans"
                >
                ...
                {user && user.id !== 'guest' && (
                  <button 
                     ...
                     className="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl hover:bg-white/5 text-white/60 hover:text-red-400 transition-colors text-sm font-medium font-sans"
                  >
                      <Icon name="x" size={18} />
                      Sign Out
                  </button>
                ...
                >
                    <div className="flex flex-col text-left">
                        <span className="text-[10px] text-white/50 uppercase tracking-wider font-semibold font-sans">Current Plan</span>
                        <div className="flex items-center gap-2">
                             <span className="text-sm font-bold text-white group-hover:text-blue-200 transition-colors font-sans">{currentTier}</span>
                             {currentTier === Tier.Free && (
                                 <span className="w-1.5 h-1.5 rounded-full bg-green-500"></span>
                             )}
                        </div>
                    </div>
                    <span className="text-xs bg-white/10 hover:bg-white/20 px-2 py-1 rounded text-white/90 transition-colors font-sans">Upgrade</span>
                </button>
            </div>
        </div>
      </div>
