...
export const PricingPage: React.FC<PricingPageProps> = ({ onBack, onSelectTier }) => {
  return (
    // Make the pricing page scrollable on small devices: top nav + content wrapper with max-height/overflow
    <div className="min-h-screen w-full bg-black text-white font-sans relative">
       ...
       {/* Navbar */}
       <nav className="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 py-6 flex items-center justify-between">
         <button 
            onClick={onBack} 
       ...
       <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-6 overflow-y-auto max-h-[calc(100vh-96px)]">
        <div className="text-center mb-8 max-w-3xl mx-auto animate-slide-up">
           ...
           {PRICING_PLANS.map((plan, idx) => {
               ...
               return (
                   <div 
                      key={plan.name}
                      className={`
                          relative flex flex-col p-6 lg:p-8 rounded-[1.25rem] border transition-all duration-300 animate-slide-up group
                          ${isPopular 
                   ...
                   >
                      {!isFree && (
                           <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none rounded-[1.25rem]">
                      ...
                      <button
                          ...
                          disabled={!isFree}
                          className={`
                              w-full py-3 rounded-xl text-sm font-bold tracking-widest transition-all
                              ${isFree
       ...
       <footer className="relative z-10 border-t border-white/10 py-6 text-center text-white/40 text-xs sm:text-sm px-4 mt-8">
